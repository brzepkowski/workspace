    ...
    public static void main(String args[]) {
        ...
        while (true) {
         ...
         // Generate vector for the second point
    		...
    		int x2, y2, x3, y3;
    		if (myPods[0][5] == checkpointCount - 1) {
    			x2 = checkpoints[0][0];
    			y2 = checkpoints[0][1];
    			x3 = checkpoints[1][0];
    			y3 = checkpoints[1][1];
    		}
    		else if (myPods[0][5] == checkpointCount - 2) {
    			x2 = checkpoints[myPods[0][5] + 1][0];
    			y2 = checkpoints[myPods[0][5] + 1][1];
    			x3 = checkpoints[0][0];
    			y3 = checkpoints[0][1];
    		}
    		else {
    			x2 = checkpoints[myPods[0][5] + 1][0];
    			y2 = checkpoints[myPods[0][5] + 1][1];
    			x3 = checkpoints[myPods[0][5] + 2][0];
    			y3 = checkpoints[myPods[0][5] + 2][1];
    		}
    		
            
            double alfa = calculateMultiplier( x0,  y0,  x1,  y1,  x2,  y2,  tAx,  tAy, 
        			 tBx,  tBy,  tCx,  tCy);
            
            int x = (int) (alfa * tAx);
            int y = (int) (alfa * tAy);
            System.out.println(x1 + " " + y1 + " 100");
         ...
         // Generate vector for the second point
    		...
    		x1 = checkpoints[myPods[0][5]][0];
    		y1 = checkpoints[myPods[0][5]][1];
            ...
            alfa = calculateMultiplier( x0,  y0,  x1,  y1,  x2,  y2,  tAx,  tAy, 
        			 tBx,  tBy,  tCx,  tCy);
            
            x = (int) (alfa * tAx);
            y = (int) (alfa * tAy);
            
            System.out.println(x2 + " " + y2 + " 100");
