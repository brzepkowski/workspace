...
def round_sig_fractional(x, sig=3):
    real, imaginary = x.real, x.imag
    frac_real, whole_real = math.modf(x.real)
    frac_imag, whole_imag = math.modf(x.imag)
    print(whole_real, ", ", frac_real, ", ", whole_imag, ", ", frac_imag)
    if abs(frac_real) != 0.0:
        frac_real = round(frac_real, sig-int(math.floor(math.log10(abs(frac_real))))-1)
    if abs(frac_imag) != 0.0:
        frac_imag = round(frac_imag, sig-int(math.floor(math.log10(abs(frac_imag))))-1)
    return complex(round(whole_real) + frac_real, round(whole_imag) + frac_imag)
...
def main():
    ...
    for i in range(0, number_of_iterations):
        ...
        if np.shape(subsystem_A_H)[0] >= math.sqrt(m):
            ...
            orders = list(map(lambda x: [round_sig_fractional(x), magnitude(x)], eigenvalues))
            print("Orders: ", orders)
