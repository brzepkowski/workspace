...
# indices if control_qubits and terget_qubit have t obe precise (not mapped or with additional starting point)
# control_qubits array has to be at least 3 elements long
...
def double_controlled_incrementer(circuit, qr, m, n, starting_index, external_control1):
    if n == 1:
        CNOT_gate(circuit, qr, m, external_control1, starting_index)
    else:
        k = math.ceil(n / 2)
        qubit_map_inverse = {i: 2*i if i < k else 2*(i-k)+1 for i in range(n)} # We can use the same interleaving method as in the CARY_gate,
        qubit_map = {v: k for k, v in qubit_map_inverse.items()} # because the initial position of ancilla and target registers is switched
        # Initial gates (CNOTs and NOTs) before first subtraction widget
        for i in range(1, n - 2, 2):
            extended_CNOT_gate(circuit, qr, m, starting_index + qubit_map[0], starting_index + qubit_map[i], external_control1)
        for i in range(2, n - 1, 2):
            extended_NOT_gate(circuit, qr, m, starting_index + qubit_map[i], external_control1)
        extended_NOT_gate(circuit, qr, m, starting_index + qubit_map[n - 1], external_control1)
        # First Subtraction Widget
        for i in range(0, n - 3, 2):
            extended_UMA_gate(circuit, qr, m, n, starting_index, starting_index + qubit_map[i], starting_index + qubit_map[i + 1], starting_index + qubit_map[i + 2], external_control1)
        extended_CNOT_gate(circuit, qr, m, starting_index + qubit_map[n - 2], starting_index + qubit_map[n - 1], external_control1)
        for i in reversed(range(0, n - 3, 2)):
            extended_MAJ_gate(circuit, qr, m, n, starting_index, starting_index + qubit_map[i], starting_index + qubit_map[i + 1], starting_index + qubit_map[i + 2], external_control1)
        # Binary negation of one of the number, which we are subtracting (written on all ancilla qubits except the 0-th one)
        for i in range(2, n - 1, 2):
            extended_NOT_gate(circuit, qr, m, starting_index + qubit_map[i], external_control1)
        # Second Subtraction Widget
        ...
        # Last CNOT gates controlled on the 0-th qubit
        for i in range(1, n - 2, 2):
            extended_CNOT_gate(circuit, qr, m, starting_index, starting_index + qubit_map[i], external_control1)
...
def shor(circuit, qr, cr, m, n, a):
    ...
    C_n_controlled_NOT_gate(circuit, qr, m, [1,2,3], 0)
