...
def TextMapper(fn:String):Unit = {
	...
	bufor.close	
	// Zapisz do pliku "intermediate_file"
	val printer  = new PrintWriter(new File("intermediate_file"),"UTF-8")
	for (x <- keyval) {
...
def TextReducer(mapper_file:String,fn:String):Unit ={
	val printer  = new PrintWriter(new File(fn),"UTF-8")

	// Odczytaj dane z pliku pośredniego (wyniku pracy Mappera)
	val bufor  = Source.fromFile(mapper_file,"UTF-8")
	var keyval = collection.mutable.ListBuffer.empty[(String,String)]
	for (line <- bufor.getLines()) {
		var temp = line.split(",")
		keyval += new Tuple2(temp(0), temp(1))
	}

	val sorted = keyval.groupBy(_._1).mapValues(_.size).toSeq.sortWith(_._2>_._2)
...
// Użyj Mappera i zapisz jego wynik do pliku pośredniego
TextMapper(args(0))
TextReducer("intermediate_file", "final_file")
